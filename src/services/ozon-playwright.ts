...
const BROWSER_PROFILE_PATH =
  process.env.OZON_BROWSER_PROFILE_PATH ??
...
async function withProfileLock<T>(task: () => Promise<T>): Promise<T> {
  ...
  profileQueue = new Promise<void>((resolve) => {
    release = resolve
  })
...
function ensureBrowserProfile(): void {
  fs.mkdirSync(BROWSER_PROFILE_PATH, { recursive: true })
}

async function seedOzonCookies(context: BrowserContext): Promise<void> {
  const cookieHeader = process.env.OZON_COOKIE?.trim()
